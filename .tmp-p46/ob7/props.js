                    ...
                    if (arterial) {
                        ...
                        const edgeWet = (t) => {
                            ...
                            for (const s of [1, -1]) {
                                ...
                                if (terrainRadius(q) < WET_MARK)
                                    return true;
                            }
                            return false;
                        };
                        while (tA > 0 && edgeWet(tA))
                            tA = Math.max(0, tA - BRIDGE_SAMPLE);
                        while (tB < omega && edgeWet(tB))
...
function nearestLoopT(dir) {
    ...
    const sx = (i) => dirs[(((i % n) + n) % n) * 3];
    const sy = (i) => dirs[(((i % n) + n) % n) * 3 + 1];
    const sz = (i) => dirs[(((i % n) + n) % n) * 3 + 2];
    let bestT = bi * step;
    let bestD2 = Infinity;
 ...
 */
function spanBanks(span, a, b, omega) {
    const probe = new THREE.Vector3();
    const bank = (t) => {
...
function bridgeHeight(span, t, bankA, bankB, deckR = BRIDGE_DECK_R) {
    if (t <= span.t0) {
 ...
 */
function spanDeckR(span, bankA, bankB) {
    if (span.halfWidth !== BRIDGE_ARTERIAL_HALF_WIDTH)
...
function placeBridges(props) {
    ...
    for (const span of BRIDGE_SPANS) {
        ...
        const pointAt = (t, target) => dirAt(t, target).multiplyScalar(bridgeHeight(span, t, bankA, bankB, deckR));
...
/** moving average over +-w samples of a CLOSED (wrapping) array */
function runningMeanWrap(src, w) {
    const n = src.length;
...
/** rolling maximum over +-w samples of a CLOSED (wrapping) array */
function rollingMaxWrap(src, w) {
    const n = src.length;
    const out = new Float64Array(n);
    for (let i = 0; i < n; i++) {
        let m = -Infinity;
        for (let k = -w; k <= w; k++) {
            const v = src[(((i + k) % n) + n) % n];
            if (v > m)
                m = v;
        }
        out[i] = m;
...
/**
 * The profile is computed ONCE around the WHOLE closed loop, wrapping, and
 * every consumer (leg meshes, ride samples, bridge banks) interpolates from
 * the same array. It used to be recomputed per leg with padded ends; the pads
 * approximated the neighbour leg instead of sharing it, which left 0.03u
 * height steps at every station seam and gave the bridges nothing to sample.
 */
let _loopProfileH = null;
let _loopProfileStep = 0;
/** cumulative WORLD arc length at each profile grid sample (nP+1 entries) */
let _loopCumS = null;
function loopProfileGrid() {
    if (_loopProfileH)
        return _loopProfileH;
    const total = NET ? NET.total : METRO_LOOP.total;
    const nP = Math.max(16, Math.ceil(total / CORRIDOR_STEP));
    _loopProfileStep = total / nP;
    const probe = new THREE.Vector3();
    const raw = new Float64Array(nP);
    for (let i = 0; i < nP; i++) {
        loopDir(i * _loopProfileStep, probe);
        raw[i] = terrainRadius(probe);
    }
    // smooth upward-biased start: the crest-clearing envelope
    let h = runningMeanWrap(rollingMaxWrap(raw, CORRIDOR_SMOOTH), CORRIDOR_SMOOTH);
    // relax toward smooth-within-bounds; the clamp runs last so the cap is exact
    for (let it = 0; it < PROFILE_ITERATIONS; it++) {
        h = runningMeanWrap(h, PROFILE_ITER_SMOOTH);
        for (let i = 0; i < nP; i++) {
            if (h[i] < raw[i])
                h[i] = raw[i];
    ...
    for (let i = 0; i < nP; i++) {
        // true surface length, radial descent included — dashes cut in r·dθ alone
        // stretched visibly on the graded approaches
        const along = _loopProfileStep * 0.5 * (h[i] + h[(i + 1) % nP]);
        const drop = h[(i + 1) % nP] - h[i];
        cum[i + 1] = cum[i] + Math.hypot(along, drop);
    }
    _loopCumS = cum;
    return h;
}
/**
 * World distance ridden along the corridor from t=0 to t, measured on the
 * profile itself. GLOBAL, so dash phase never resets at a leg seam — the
 * per-leg run it replaces restarted at every station and the dashes bunched.
 */
function loopWorldS(t) {
    loopProfileGrid();
    const cum = _loopCumS;
    const nP = cum.length - 1;
    // unwrapped: the final leg's t runs past the closed loop's seam, and a
    // mid-leg reset to zero would tear its dash phase apart
    const totalAngular = nP * _loopProfileStep;
    const wraps = Math.floor(t / totalAngular);
    const x = (t - wraps * totalAngular) / _loopProfileStep;
    const i = Math.min(nP - 1, Math.floor(x));
    const f = x - i;
    return wraps * cum[nP] + cum[i] + (cum[i + 1] - cum[i]) * f;
}
const _profProbe = new THREE.Vector3();
/** rideable corridor height at loop arc position t — seam-free, cap exact */
function loopProfileAt(t) {
    const h = loopProfileGrid();
    const nP = h.length;
    let x = t / _loopProfileStep;
    x = ((x % nP) + nP) % nP;
    const i = Math.floor(x);
    const f = x - i;
    let v = h[i] + (h[(i + 1) % nP] - h[i]) * f;
    // re-clamp at the query point so the fill cap holds exactly everywhere,
    // not just at grid points
    const raw = terrainRadius(loopDir(t, _profProbe));
    if (v < raw)
        v = raw;
    else if (v > raw + ROAD_MAX_FILL)
        v = raw + ROAD_MAX_FILL;
    return v;
}
function corridorProfile(tA, span, steps) {
    const out = [];
    for (let i = 0; i <= steps; i++)
        out.push(loopProfileAt(tA + (i / steps) * span));
    return out;
}
    ...
    legsData.forEach(({ ai, samples, steps }) => {
        ...
        const dashSpans = (sA, sB) => {
            ...
            for (let k = Math.floor(sA / DASH_PERIOD); k * DASH_PERIOD < sB; k++) {
                ...
                if (hi > lo + 1e-4)
                    out.push([(lo - sA) / (sB - sA), (hi - sA) / (sB - sA)]);
            }
            return out;
        };
        for (let i = 0; i < steps; i++) {
            ...
            for (const sign of [-1, 1]) {
                ...
                for (const [f0, f1] of dashes) {
                    ...
                    flat(paint, d0, d1, sign * LANE_MID - MARK_HALF, sign * LANE_MID + MARK_HALF, PAINT_LIFT);
                }
                // footpath
...
function corridorSurfaceAt(samples, bestI, dir, requireSegment = false) {
    ...
    if (s0 !== s1) {
        ...
        const len2 = _rideAB.lengthSq();
        if (len2 > 1e-12)
            right.addScaledVector(_rideAB, -right.dot(_rideAB) / len2);
    }
