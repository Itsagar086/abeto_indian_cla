...
import { surfacePoint, surfaceQuaternion, rng, roadDistance, terrainRadius, terrainColor, npcSurfacePosition, slopeAt, } from "./terrain.js";
const PALETTE = {
    bazaar: [
        ["#d97b3a", "#f2d9a8"],
        ["#c25959", "#f2e6d0"],
...
function planLoop() {
    ...
    const dist = (a, b) => ZONE_DIRS[a].angleTo(ZONE_DIRS[b]);
...
export function buildCorridors(props) {
    ...
    legsData.forEach(({ ai, samples, steps }) => {
         ...
         */
        const shoulderPoint = (s, o, fOverride) => {
            const probe = s.dir.clone().addScaledVector(s.right, o / s.ground).normalize();
            ...
            const h = fOverride === 1
                ? rawH
                : shoulderHeight(s.ground, terrainRadius(eProbe), rawH, Math.abs(o));
            return s.dir.clone().addScaledVector(s.right, o / h).normalize().multiplyScalar(h);
        };
        const skirtSide = (s0, s1, sign) => {
            const trimmed = foldTrim(s0, s1, Math.min(sign * FOOT_OUT, sign * SKIRT_OUT), Math.max(sign * FOOT_OUT, sign * SKIRT_OUT), true);
            if (!trimmed)
                return;
            const inner = sign === 1 ? trimmed[0] : trimmed[1];
            const outer = sign === 1 ? trimmed[1] : trimmed[0];
            const outerTrimmed = Math.abs(outer) < SKIRT_OUT - 1e-4;
            ...
            for (let k = 0; k <= STRIPS; k++) {
                const o = inner + ((outer - inner) * k) / STRIPS;
                const fo = k === STRIPS && outerTrimmed ? 1 : undefined;
                rows0.push(shoulderPoint(s0, o, fo));
                rows1.push(shoulderPoint(s1, o, fo));
                cols0.push(groundColour(s0, o));
                cols1.push(groundColour(s1, o));
            }
            for (let k = 0; k < STRIPS; k++) {
...
export function corridorSurface(dir) {
    ...
    if (a > FOOT_OUT) {
        // the walkable shoulder: the SAME curve the skirt mesh is built from
        const eDir = _rideProbe
            .copy(dir)
            .addScaledVector(right, (Math.sign(o) * FOOT_OUT - o) / ground)
            .normalize();
        return shoulderHeight(ground, terrainRadius(eDir), raw, a);
    }
 ...
 */
function shoulderHeight(ground, edgeRaw, pointRaw, a) {
    const limit = FOOT_OUT * Math.tan(MAX_TWIST);
    const edgeH = ground + Math.max(-limit, Math.min(limit, edgeRaw - ground)) + FOOTPATH_TOP;
    const f = smoothstep01((a - FOOT_OUT) / SHOULDER_FALLOFF);
