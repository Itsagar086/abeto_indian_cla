...
export function loopProfileAt(t) {
    const h = profileGrid();
    const { dirs, n, step } = METRO_LOOP;
    let x = t / step;
    x = ((x % n) + n) % n;
    ...
    _lpProbe
        .set(dirs[i * 3] + (dirs[j * 3] - dirs[i * 3]) * f, dirs[i * 3 + 1] + (dirs[j * 3 + 1] - dirs[i * 3 + 1]) * f, dirs[i * 3 + 2] + (dirs[j * 3 + 2] - dirs[i * 3 + 2]) * f)
        .normalize();
