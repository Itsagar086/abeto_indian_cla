...
export type SignLine = { text: string; font: string; color: string }
...
function withPx(font: string, px: number) {
 ...
 */
export function canRenderKannada(ctx: CanvasRenderingContext2D) {
  ctx.save()
...
export function makeSignTexture(
  ...
  const fitted = usable.map((l) => {
    ...
    for (let i = 0; i < 12; i++) {
      ctx.font = font
      if (ctx.measureText(l.text).width <= inner) break
      px -= 3
