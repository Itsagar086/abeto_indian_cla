...
import { surfacePoint, surfaceQuaternion, rng, roadDistance, terrainRadius, terrainColor, npcSurfacePosition, slopeAt, METRO_LOOP, ROAD_MAX_FILL, loopProfileAt, loopWorldS, } from "./terrain.js";
const PALETTE = {
    bazaar: [
        ["#d97b3a", "#f2d9a8"],
        ["#c25959", "#f2e6d0"],
    ...
    ZONES.forEach((zone, zi) => {
        ...
        for (let attempt = 0; attempt < attempts.length; attempt++) {
            ...
            for (let k = SIGN_OFFSET; k <= 0.26; k += 0.03) {
                ...
                if (roadDistance(cand) < RIBBON_CLEAR)
                    continue;
                if (arterialDistance(cand) < CORRIDOR_SUPPRESS)
                    continue;
                dir = cand;
                break;
            }
 ...
 */
function corridorSampleDry(dir, right, ground) {
    ...
    if (terrainRadius(dir) < WATER_LEVEL + 0.48)
        return false;
    if (terrainRadius(_dryProbe.copy(dir).addScaledVector(right, FOOT_OUT / ground).normalize()) <
        WATER_LEVEL + 0.48)
        return false;
    if (terrainRadius(_dryProbe.copy(dir).addScaledVector(right, -FOOT_OUT / ground).normalize()) <
        WATER_LEVEL + 0.48)
