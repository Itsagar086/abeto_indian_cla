/**
 * World props — placement layout and colour palettes for the Dak Wala world.
 *
 * Edit PALETTE to change the colours of buildings and props per zone.
 * The prop layout (which kinds go where, at what angle/distance/scale)
 * is defined in the `buildProps` function below.
 *
 * To add a new prop kind:
 *   1. Add its id to the PropKind union.
 *   2. Add its rendering mesh in components/game/PropsLayer.tsx.
 *   3. Add placement calls inside buildProps().
 *
 * No logic files need to change when editing palette colours or prop counts.
 */

import * as THREE from "three"
import { ZONES } from "../data/zones"
import { surfacePoint, surfaceQuaternion, rng } from "../utilities/helpers"
 ...
 * Each zone maps to a list of [colorA, colorB] pairs.
 * The pair used for each prop is chosen deterministically from the seed.
 *
 * Edit colours here to change the look of each zone.
 */
const PALETTE: Record<string, [string, string][]> = {
  bazaar:   [["#d97b3a", "#f2d9a8"], ["#c25959", "#f2e6d0"], ["#3f7f8c", "#f2e6d0"], ["#9c6ea0", "#f2e6d0"]],
...
export function buildProps(): PlacedProp[] {
  ...
  millAngles.forEach((a, i) =>
    add("mill-block", "mill", a, 1.3 + (i % 2) * 0.3, 1.3, 400 + i),
  )

  // --- workshop: single shed + lamp
  ...
  // --- beach: leaning palms (mango-tree reused at thin scale)
  const beach = rng(77)
  for (let i = 0; i < 6; i++) {
